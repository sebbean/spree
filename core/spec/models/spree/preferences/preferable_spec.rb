...
describe Spree::Preferences::Preferable do
  ...
  describe "preference definitions" do
    it "parent should not see child definitions" do
      expect(@a.has_preference?(:color)).to eq(true)
      expect(@a.has_preference?(:flavor)).to eq(false)
    end

    it "child should have parent and own definitions" do
      expect(@b.has_preference?(:color)).to eq(true)
      expect(@b.has_preference?(:flavor)).to eq(true)
    end
    ...
    it "can be asked if it has a preference definition" do
      expect(@a.has_preference?(:color)).to eq(true)
      expect(@a.has_preference?(:bad)).to eq(false)
    end
  ...
  describe "preference access" do
    ...
    context "converts boolean preferences to boolean values" do
      ...
      it "with strings" do
        ...
        expect(@a.preferences[:is_boolean]).to eq(false)
        @a.set_preference(:is_boolean, 'f')
        expect(@a.preferences[:is_boolean]).to eq(false)
        @a.set_preference(:is_boolean, 't')
        expect(@a.preferences[:is_boolean]).to eq(true)
      end
  ...
  it "can add and remove preferences" do
    ...
    expect(@a.preferred_test_temp).to eq(true)
    A.remove_preference :test_temp
    expect(@a.has_preference?(:test_temp)).to eq(false)
    expect(@a.respond_to?(:preferred_test_temp)).to eq(false)
  end
