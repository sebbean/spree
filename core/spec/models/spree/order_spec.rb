...
describe Spree::Order do
  ...
  context "#products" do
    ...
    it "contains?" do
      expect(order.contains?(@variant1)).to eq(true)
    end
  ...
  context "#generate_order_number" do
    it "should generate a random string" do
      expect(order.generate_order_number.is_a?(String)).to eq(true)
      expect(order.generate_order_number.to_s.length > 0).to eq(true)
    end
  end
  ...
  context "#can_ship?" do
    ...
    it "should be true for order in the 'complete' state" do
      ...
      expect(order.can_ship?).to eq(true)
    end
    ...
    it "should be false if the order is neither in the 'complete' nor 'resumed' state" do
      ...
      expect(order.can_ship?).to eq(false)
    end
  end
  ...
  context "#process_payments!" do
    ...
    it "should return false if no pending_payments available" do
      ...
      expect(order.process_payments!).to eq(false)
    end
    ...
    context "when a payment raises a GatewayError" do
      ...
      it "should return true when configured to allow checkout on gateway failures" do
        ...
        expect(order.process_payments!).to eq(true)
      end
  ...
  context "#allow_checkout?" do
    it "should be true if there are line_items in the order" do
      ...
      expect(order.checkout_allowed?).to eq(true)
    end
    it "should be false if there are no line_items in the order" do
      order.stub_chain(:line_items, :count => 0)
      expect(order.checkout_allowed?).to eq(false)
    end
  end
  ...
  context "insufficient_stock_lines" do
    ...
    it "should return line_item that has insufficient stock on hand" do
      ...
      expect(order.insufficient_stock_lines.include?(line_item)).to eq(true)
    end
  ...
  context "payment required?" do
    ...
    context "total is zero" do
      it { expect(order.payment_required?).to eq(false) }
    end

    context "total > zero" do
      before { order.stub(total: 1) }
      it { expect(order.payment_required?).to eq(true) }
    end
  end
