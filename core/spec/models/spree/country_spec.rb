...
describe Spree::Country do
  it "can find all countries group by states required" do
    ...
    expect(states_required[country_states_required.id.to_s]).to eq(true)
    expect(states_required[country_states_not_required.id.to_s]).to eq(false)
  end

  it "returns that the states are required for an invalid country" do
    expect(Spree::Country.states_required_by_country_id['i do not exit']).to eq(true)
  end
end
